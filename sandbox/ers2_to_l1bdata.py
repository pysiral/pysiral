...
def ers2_to_l1bdata():
    ...
    # Get an L1B SAR file
    l1b_directory = config.local_machine.l1b_repository.ers2.sgdr
    l1b_directory = os.path.join(l1b_directory, "1996", "03", "01")
    ...
    l1b.construct()
    t1 = time.time()
    print "Constructing ERS-2 l1bdata object in %.3g seconds" % (t1 - t0)

    # Quick plots
    ers2_l1b_fullorbit_plot(l1b)

    # Limit amounts of waveforms to plot => memory error
    l1b.trim_to_subset(np.arange(1000)+9000)

    # Quick plots
    ers2_l1b_orbit_plot(l1b)
    ers2_l1b_corrections_plot(l1b)
    ers2_l1b_waveform_plot(l1b)


def ers2_l1b_orbit_plot(l1b):
...
#        lat_0 *= -1.0
    ...
    plt.figure("ERS-2 Orbit Quickview", facecolor="white")
...
def ers2_l1b_fullorbit_plot(l1b):
...
def ers2_l1b_corrections_plot(l1b):
...
def ers2_l1b_waveform_plot(l1b):

    plt.figure("ERS-2 Waveform Quickview",
