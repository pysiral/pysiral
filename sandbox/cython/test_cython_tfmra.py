...
from pysiral.retracker import TFMRA, cTFMRA
...
def test_cython_tfmra():
    ...
    print ". number of waveforms: %g " % l1b.n_records
    ...
    ctfmra.init(l1b.n_records)
    ctfmra.l2_retrack(rng, wfm, indices, rmode, is_valid)
    t2 = time.clock()
    print " . done in %.2f seconds" % (t2 - t1)

    kpi = np.std(ctfmra.range-tfmra.range)
    print "sdev(ctfrma.range - tfrma.range) = %.9f m" % kpi

    import matplotlib.pyplot as plt
    plt.plot(tfmra.range, color="black", lw=2, label="TFMRA")
    plt.plot(ctfmra.range, color="red", label="cTFMRA")
    plt.legend()
    plt.show()
