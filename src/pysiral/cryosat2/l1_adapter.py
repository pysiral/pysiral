...
from pysiral.core.config import AttrDict
...
class ESACryoSat2ICEBaselineEL1b(Level1PInputHandlerBase):
    ...
    def translate_opmode2radar_mode(op_mode: str) -> str:
        ...
        :param op_mode: One of `lrm`, `sar`, or `sarin`

        :return: One of `lrm`, `sar, or `sin`
        """
        translate_dict = {"sarin": "sin"}
    ...
    def get_l1(
        ...
        # Polar ocean check passed, now fill remaining l1 data groups
    ...
    def _read_input_netcdf(self, filepath) -> Optional[xarray.Dataset]:
        """
        Read the netCDF file via xarray and return the daaset as

        :param filepath: The full filepath to the netCDF file

        :return: xarray.Dataset or None when errors have occured
        """
        try:
            return xarray.open_dataset(filepath, decode_times=False, mask_and_scale=True)
        except OSError as e:
            msg = f"OSError encountered by xarray: {filepath} {str(e)}"
            self.error.add_error("xarray-parse-error", msg)
            logger.error(msg)
        except Exception as e:
            msg = f"Error encountered by xarray: {filepath} {str(e)}"
            self.error.add_error("xarray-parse-error", msg)
            logger.error(msg)
            return None

    def _set_input_file_metadata(self, nc: xarray.Dataset, l1: Level1bData) -> None:
        """
        Extract product metadata from global attributes

        :param nc: The content of the Level-1b netCDF file as xarray.Dataset
    ...
    def _set_l1_data_groups(self, nc: xarray.Dataset, l1: Level1bData):
        """
    ...
    def _set_time_orbit_data_group(self, nc: xarray.Dataset, l1: Level1bData) -> None:
        """
